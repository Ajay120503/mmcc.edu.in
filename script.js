...
// JavaScript code for creating a slideshow

// Function to create and initialize the slideshow
function createSlideshow() {
    const slideBoxes = document.querySelectorAll('.slide-box'); // Select all slide-box elements
    let currentSlideIndex = 0; // Initialize current slide index

    // Function to show the current slide
    function showSlide(index) {
        // Hide all slide-box elements
        slideBoxes.forEach(slide => slide.style.display = 'none');
    ...
    function nextSlide() {
        currentSlideIndex++; // Increment current slide index
        if (currentSlideIndex >= slideBoxes.length) {
            currentSlideIndex = 0; // Reset index to loop back to the first slide
        }
        showSlide(currentSlideIndex); // Display the next slide
    }
    ...
    setInterval(nextSlide, 5000); // Adjust interval time as needed
}

// Call the function to create and initialize the slideshow
createSlideshow();
